...
void visualizeMPOPITrajectories() {
    ...
    optimal_path.header.frame_id = "base_link";  // trajectories가 ENU → map 프레임 정확
    ...
    reference_path.header.frame_id = "base_link";  // ← "map" → "base_link"
    reference_path.header.stamp = ros::Time::now();

    for (int i = 0; i < (int)waypoints.size(); i++) {
        // ENU → base_link 변환
        Point2D enu_pt{waypoints[i].x, waypoints[i].y};
        Point2D bl_pt;
        mapToBaseLink(enu_pt, ego, bl_pt);  // ← 추가

        geometry_msgs::PoseStamped pose;
        pose.header.frame_id = "base_link";  // ← "map" → "base_link"
        pose.header.stamp = ros::Time::now();
        pose.pose.position.x = bl_pt.x;     // ← waypoints[i].x → bl_pt.x
        pose.pose.position.y = bl_pt.y;     // ← waypoints[i].y → bl_pt.y
        pose.pose.position.z = 0.0;
    ...
    for (int n = 0; n < top_n; n++) {
        ...
        line_marker.header.frame_id = "base_link";  // ENU → map 프레임 정확
