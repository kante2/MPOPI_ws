...
//   // lidar law data, timestamp (측정한 센서 값)
//   // * 구조체 MeasurementPackage 내용 
//   //   : 센서 타입, 물체 위치 (x, y) 좌표 데이터, 데이터가 측정된 시각 (timestamp) -> 3가지가 리스트에 담김

//   vector<GroundTruthPackage> gt_pack_list;
//   // 물체의 실제 맵 상의 값

//   ReadMeasurements(in_file_name, measurement_pack_list, gt_pack_list);
//   // date file(in_file_name) 로부터 센서 측정값과 실제값을 읽어서 각각의 리스트에 저장 ( = 데이터 준비 )
//   // -> 나는
//   // 센서 측정값과 실제값을 바로 위처럼 생성해둔 vector 에 저장

//   RunEkf(measurement_pack_list, gt_pack_list, out_file_name);
//   // ReadMeasurements() 로 준비한 데이터를 이용해서 필터링하고 결과 기록
// }
...
//     exit(EXIT_FAILURE);
//   }
//   // 필터가 계산한 추정값, 센서 측정값, 실제값을 txt파일에 기록
//   // -> 나는 결과를 struct에 기록? 
...
//     // =========================================================
//     // 1. kalman filter 실행 -> process.cpp 
//     // =========================================================

//     fusionEKF.ProcessMeasurement(measurement_pack_list[k]);
//     // init -> predict -> update 실행
    
//     // : 첫 데이터일 때 - 초기화 (상태변수 x에 첫 위치 저장)
//     // : 두 번째 데이터부터 - 시간 간격 dt 계산 -> predict -> update
...
//     // 필터가 계산해낸 현재 위치와 속도 (x0, x1, x2, x3) 을 파일에 기록 (-> 나는 struct 에 저장)
//     out_file << x(0) << "\t";
//     out_file << x(1) << "\t";
//     out_file << x(2) << "\t";
//     out_file << x(3) << "\t";

//     // 비교를 위해 raw measurement 도 기록
...
//     // =========================================================
//     // 3. 추정값과 데이터셋에 포함된 실제 측정값(위치, 속도) ground truth 을 각각 리스트에 저장 
//     // =========================================================

//     estimations.push_back(fusionEKF.GetEstimations());
//     ground_truth.push_back(gt_pack_list[k].gt_values_);
//     // 필터로 예측이 끝난 이후에 estimations, ground_truth 리스트를 Calculate() 에 통째로 넘겨서 
//     // 칼만필터의 평균 오차가 출력됨. 
